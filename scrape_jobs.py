...
# ──────────────────────────────────────────────
# PRE-FILTER — free keyword check before Claude
# ──────────────────────────────────────────────
MUST_HAVE_ANY = [
    "python", "ai", "ml", "machine learning", "llm", "gpt", "langchain",
    "fastapi", "backend", "software engineer", "cloud", "aws", "azure",
    "sre", "devops", "platform", "full stack", "fullstack", "node",
    "react", "rag", "generative", "deep learning", "nlp"
]

HARD_EXCLUDE = [
    "attorney", "lawyer", "legal", "teacher", "nurse", "doctor", "medical",
    "sales", "marketing", "accountant", "finance", "hr manager", "recruiter",
    "designer", "graphic", "content writer", "copywriter", "electrician",
    "mechanical", "civil engineer", "construction", "driver", "chef"
]

def pre_filter(job):
    """Fast local check — no API call. Returns True if job is worth analyzing."""
...
def save_to_notion(job, analysis):
    def rt(text):
        return [{"text": {"content": str(text or "")[:800]}}]
    ...
    payload = {
        ...
        "children": [
            ...
            },
            {
                "object": "block",
                "type": "paragraph",
                "paragraph": {"rich_text": [{"text": {"content": job["description"][:800]}}]},
            },
        ],
    }
...
def main():
    ...
    for i, job in enumerate(new_jobs, 1):
        ...
        if not pre_filter(job):
            print("  ⏭  Pre-filter: not relevant enough")
            skipped += 1
            continue
